...
class StarboardCog(commands.Cog, name="Starboard"):
    ...
    @starboard.command(name='fakedata', hidden=True)
    @commands.is_owner()
    async def fake_data(self, ctx, entries: int = 100):
        """A command to create fake starboard entries for testing"""
        class Channel:
            id = 00000000
...
class StarboardPages(FieldPages):
    def __init__(self, ctx, *, entries):
        super().__init__(ctx, entries=entries)
        self.embed = CustomEmbeds.info(author="Starboards")
